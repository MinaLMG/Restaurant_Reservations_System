...
async def getTables(db: db_dependency, current_user: UserReduced = Depends(verify_token)):
...
async def getSlots(db: db_dependency, current_user: UserReduced = Depends(verify_token)):
